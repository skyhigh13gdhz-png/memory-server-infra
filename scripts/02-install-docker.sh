...
report(){ echo "========== Docker 安装前检测 =========="; echo "系统       : ${OS_ID} ${OS_VERSION} (${OS_CODENAME})"; echo "架构       : ${ARCH}"; echo "Docker CLI : $(command -v docker 2>/dev/null || echo 未安装)"; echo "Docker服务 : $(systemctl is-active docker 2>/dev/null || echo 未安装/未运行)"; echo "Compose    : $(docker compose version 2>/dev/null || echo 未安装)"; echo "Xray       : $(systemctl is-active xray 2>/dev/null || echo 未安装/未运行)"; echo "========================================"; }
preflight(){
...
EOF
  ...
  # apt-cache policy 的字段名会随系统 locale 翻译（中文环境显示“候选”，不是 Candidate）。
  # 强制 C locale，并提供 madison 第二检测路径，避免把“仓库已有包”误判成“无候选版本”。
  local candidate=""
  candidate="$(LC_ALL=C apt-cache policy docker-ce 2>/dev/null | awk '/Candidate:/ {print $2; exit}')"
  if [[ -z "$candidate" || "$candidate" == "(none)" ]]; then
    candidate="$(LC_ALL=C apt-cache madison docker-ce 2>/dev/null | awk 'NR==1 {gsub(/^ +| +$/, "", $3); print $3}')"
  fi
  if [[ -z "$candidate" || "$candidate" == "(none)" ]]; then
    echo "---- Docker apt 诊断 ----" >&2
    LC_ALL=C apt-cache policy docker-ce >&2 || true
    LC_ALL=C apt-cache madison docker-ce | head -n5 >&2 || true
